...
class RedisScoreboardMoniter():
    ...
    def run(self):
        while (True):
            for dataset_letter in DatasetLetter.__iter__():
                ...
                for rank,score in enumerate(self.r.zrange(Dataset.get_dataset_score_container_key(dataset_letter),0,5,withscores=True)):
    ...
    def write_top_scores_to_file(self):
        for dataset_letter in DatasetLetter.__iter__():
            top_ss_id = self.r.zrange(Dataset.get_dataset_score_container_key(dataset_letter), 0, 0)
